...
namespace ww::build
{
    namespace
    {
        ...
        // The landmark tables cover the baked area graph only — its walk and
        // point-to-point-transition edges. Global teleports are deliberately
        // absent, and their absence costs nothing in admissibility: the runtime
        // uses a global teleport only as a start-frontier seed, never as a
        // mid-search edge (ADR 0009, and see runtime/AltHeuristic.h), so the
        // cost metric the search minimizes is exactly the one these tables
        // bound. Omitting an edge can only lengthen a true path, which leaves a
        ...
        // Areas incident to at least one edge — the only areas reachable from
        // elsewhere in the abstract graph, so the only sensible landmark
        // candidates.
